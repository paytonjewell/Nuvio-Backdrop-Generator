...
import { fetchFilterImages, fetchTraktImages, loadImages, shuffle } from './lib/tmdb'
...
export default function App() {
  ...
  const [traktKey, setTraktKey] = useState(() => localStorage.getItem('trakt_key') || '')
  ...
  const [status, setStatus] = useState({ state: '', message: 'Enter your TMDB API key and pick a source above.' })
  ...
  const generate = useCallback(async () => {
    if (!tmdbKey) { setStatus({ state: 'error', message: 'Please enter your TMDB API key first.' }); return }
    setGenerating(true)
    ...
    try {
      ...
      if (paths.length === 0) {
        setStatus({ state: 'error', message: 'No images found — try a different filter or IDs.' })
        setGenerating(false)
        return
      }

      setStatus({ state: 'loading', message: `Loading ${paths.length} images…` })
  ...
  return (
    <div className={s.app}>
      <header className={s.header}>
        ...
        <span className={s.subtitle}>— streaming-style hero images from TMDB</span>
      </header>
      ...
      <div className={s.layout}>
        <aside className={s.sidebar}>
          ...
          <div className={s.actions}>
            ...
            <SecondaryButton onClick={reshuffleImages} disabled={images.length === 0}>
              Shuffle Images
            </SecondaryButton>
