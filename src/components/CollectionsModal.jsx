...
export default function CollectionsModal({
  ...
  // Render canvas as soon as we enter folder detail
  useEffect(() => {
    if (step !== "folder" || !images.length) return;
  ...
  }, [step, images, buildSettings, text, excludedPaths]);

  // Drag handlers for comparison slider — window-level so drag works outside the container
  useEffect(() => {
    const move = (clientX) => {
        ...
        return {
          ...
          folders: col.folders.map((f) =>
            f.id !== selectedFolder.id
              ? f
              : { ...f, heroBackdropUrl: hostedUrl },
          ),
        };
      });
      await pushCollections(
        accessToken,
        selectedProfile.profile_index,
        newCollectionsJson,
      );
  ...
  const hasCurrent = !!selectedFolder?.heroBackdropUrl;
  const hasNew = !!newPreviewUrl;
  const showSlider = hasCurrent && hasNew;
      ...
      >
        <div className={s.header}>
          <div className={s.headerLeft}>
            ...
            <span className={s.title} id="collections-modal-title">
              {title}
            </span>
          </div>
                ...
                {profiles.map((p) => (
                  ...
                  >
                    {p.avatar_url ? (
                      ...
                      <div
                        className={s.avatar}
                        style={{ background: p.avatar_color_hex ?? "#6c63ff" }}
                      >
            ...
            {loading ? (
              ...
              <div className={s.collectionsScroll}>
                ...
                {collections.length === 0 && !error && (
                  <p className={s.hint}>
                    No collections found for this profile.
                  </p>
                )}
              </div>
            )}
        ...
        {step === "folder" && selectedFolder && (
          <div className={s.folderDetailWrap}>
            {/* Comparison slider */}
            <div ref={compareRef} className={s.compareContainer}>
              {/* New image — base layer (right side) */}
              {hasNew ? (
                <img
                ...
                <div className={s.compareEmpty}>
                  {images.length === 0
                    ? "Generate a backdrop first"
                    : "Rendering…"}
                </div>
              )}

              {/* Current image — clipped to left of slider */}
              {hasCurrent && (
                <img
                  className={s.compareImg}
              ...
              {/* Labels — wrapped in full-container divs so clip-path % uses container dims, not label dims */}
              {hasCurrent && (
                <div
                  className={s.compareLabelLayer}
                  style={{ clipPath: `inset(0 ${100 - sliderPos}% 0 0)` }}
                >
                  <span className={s.compareLabel} style={{ left: 12 }}>
                    Current
                  </span>
                </div>
              )}
              {hasNew && (
                ...
                >
                  <span className={s.compareLabel} style={{ right: 12 }}>
                    New
                  </span>
                </div>
              )}

              {/* Drag handle — only when both images are ready */}
              {showSlider && (
                <div
              ...
              {/* Rendering indicator */}
              {rendering && <div className={s.renderingBadge}>Rendering…</div>}
            </div>

            <div className={s.folderDetailMeta}>
              <div
                className={`${s.folderThumb} ${selectedFolder.tileShape === "POSTER" ? s.folderThumbPoster : ""}`}
              >
                {selectedFolder.coverImageUrl && !thumbImgFailed ? (
                  ...
                  <div className={s.folderThumbEmoji}>
                    {selectedFolder.coverEmoji}
                  </div>
                ) : (
              ...
              <div className={s.folderDetailInfo}>
                <span className={s.folderDetailTitle}>
                  {selectedFolder.title}
                </span>
                <span className={s.folderDetailSub}>
